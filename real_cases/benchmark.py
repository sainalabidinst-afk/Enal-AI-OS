...
def _derive_expected_findings(expected_inner: dict[str, Any], tags: list[str]) -> list[str]:
    ...
    tag_to_finding = {
        ...
        # Extended tag coverage for network cases
        "password": ["password", "default password", "weak password", "user password"],
        "weak-password": ["password", "weak password", "default password"],
        "admin-exposed": ["admin", "exposed", "management", "unrestricted", "open to the world"],
        "service-exposure": ["service", "exposed", "restrict", "open to the world"],
        "api": ["api", "restrict", "open to the world"],
        "web": ["web", "http", "www", "unrestricted"],
        "backup": ["backup", "configuration backup"],
        "ppp": ["ppp", "encryption", "pppoe"],
        "pptp": ["pptp", "vpn", "remote access"],
        "ipsec": ["ipsec", "vpn", "crypto", "tunnel"],
        "site-to-site": ["ipsec", "vpn", "site-to-site", "tunnel"],
        "remote-access": ["remote access", "vpn", "ipsec"],
        "dns": ["dns", "name-server", "remote requests"],
        "snmp": ["snmp", "acl", "community"],
        "ntp": ["ntp", "authenticate", "time"],
        "hsrp": ["hsrp", "standby", "vrrp", "high availability"],
        "redundancy": ["redundancy", "failover", "ha", "hsrp", "vrrp"],
        "cluster": ["cluster", "ha", "high availability", "failover"],
        "stp": ["stp", "spanning-tree", "bridge", "portfast"],
        "switching": ["switchport", "vlan", "trunk", "switching"],
        "trunk": ["trunk", "switchport", "vlan"],
        "layer2": ["vlan", "switchport", "trunk", "bridge", "stp"],
        "corporate": ["ssid", "wireless", "wlan", "wpa"],
        "employee": ["ssid", "wireless", "wlan", "wpa"],
        "wifi": ["wireless", "wlan", "ssid", "wpa"],
        "wpa": ["wpa", "wpa2", "wpa3", "enterprise", "wireless"],
        "wpa2": ["wpa2", "wpa", "wireless", "encryption"],
        "ssid": ["ssid", "wireless", "wlan"],
        "dot11": ["dot11", "wireless", "ssid", "wpa"],
        "port-forward": ["nat", "port", "forward", "dnat"],
        "pat": ["nat", "pat", "port", "overload"],
        "vip": ["vip", "virtual", "nat", "load balance"],
        "masquerade": ["nat", "masquerade", "srcnat"],
        "dmz": ["dmz", "nat", "firewall", "access"],
        "ingress": ["ingress", "firewall", "input chain", "access"],
        "wan-protection": ["wan", "firewall", "input", "protect"],
        "input-filter": ["input chain", "firewall", "filter"],
        "telephony": ["voice", "voip", "sip", "priority"],
        "voice": ["voice", "voip", "sip", "priority", "qos"],
        "voip": ["voice", "voip", "sip", "priority", "qos"],
        "priority": ["priority", "qos", "queue", "class"],
        "shaper": ["queue", "shaping", "traffic", "limit"],
        "traffic-shaping": ["queue", "traffic", "shaping", "limit"],
        "queue": ["queue", "limit", "traffic", "qos"],
        "policy": ["policy", "firewall", "access control"],
        "static": ["static route", "route", "gateway"],
        "services": ["service", "ntp", "dns", "snmp", "restrict"],
        "managed-switch": ["vlan", "switchport", "trunk", "managed"],
        "catalyst": ["switchport", "vlan", "trunk", "spanning-tree"],
        "cisco": ["security", "routing", "vlan", "vpn", "firewall"],
        "fortinet": ["firewall", "policy", "security", "vpn"],
    }
    for tag in tags:
